...
`vercel.json` pins the serverless function to `fra1` (Frankfurt) to match the Turso database's region — see "Deploy for free" above for why colocating the two matters more than picking a region close to any one visitor. If you ever move your Turso database to a different region, update the `regions` value in `vercel.json` to match (Turso's region codes map directly to Vercel's — `fra`→`fra1`, `cdg`→`cdg1`, `hnd`→`hnd1`, and so on). Some latency on Vercel's Hobby tier is unavoidable regardless — serverless functions "cold start" after sitting idle, and the first request after a gap is always slower.
